...
// Main transcode context that holds all state
export interface TranscodeContext {
    ifmt_ctx: FormatContext;
    ofmt_ctx: FormatContext;
    stream_ctx: StreamContext[];
    filter_ctx: FilteringContext[];
}

export async function open_input_file(filename: string): Promise<{ ret: number; ctx: FormatContext; streamCtx: StreamContext[] }> {
    let ret: number;
    const ifmt_ctx = new FormatContext();
    ...
    if (ret < 0) {
        ...
        return { ret, ctx: ifmt_ctx, streamCtx: [] };
    }
    ...
    const stream_ctx: StreamContext[] = new Array(nb_streams);
    ...
    for (let i = 0; i < nb_streams; i++) {
        ...
        if (!dec) {
            ...
            return { ret: -1, ctx: ifmt_ctx, streamCtx: stream_ctx };
        }
        ...
        if (ret < 0) {
            ...
            return { ret, ctx: ifmt_ctx, streamCtx: stream_ctx };
        }
    ...
    return { ret: 0, ctx: ifmt_ctx, streamCtx: stream_ctx };
}

export async function open_output_file(input_ctx: FormatContext, input_stream_ctx: StreamContext[], io_ctx: IOContext): Promise<{ ret: number; ctx: FormatContext }> {
    let ret: number;


    const ofmt_ctx = new FormatContext();

    // fMP4 options for streaming
    ret = ofmt_ctx.allocOutputContext2(null, 'mp4', null);

    if (ret < 0) {
        console.error('Could not create output context');
        return { ret, ctx: ofmt_ctx };
    }

    ofmt_ctx.setOption('movflags', 'frag_keyframe+empty_moov+default_base_moof+frag_every_frame+omit_tfhd_offset');

    for (let i = 0; i < input_ctx.nbStreams; i++) {
        const out_stream = ofmt_ctx.newStream(null);
        if (!out_stream) {
            console.error('Failed allocating output stream');
            return { ret: -1, ctx: ofmt_ctx };
        }

        const in_stream = input_ctx.streams![i];
        const dec_ctx = input_stream_ctx[i].dec_ctx;
        ...
        if (dec_ctx && (dec_ctx.codecType === AVMEDIA_TYPE_VIDEO || dec_ctx.codecType === AVMEDIA_TYPE_AUDIO)) {
            ...
            input_stream_ctx[i].enc_ctx = enc_ctx;
    ...
    return { ret: 0, ctx: ofmt_ctx };
}

export async function init_filter(dec_ctx: CodecContext, enc_ctx: CodecContext, filter_spec: string): Promise<{ ret: number; filteringCtx: FilteringContext }> {
    let args: string;
    let ret = 0;
    ...
    if (dec_ctx.codecType === AVMEDIA_TYPE_VIDEO) {
        ...
        if (ch_layout?.order === undefined) {
            ...
            const mask = ch_layout?.nbChannels === 1 ? 4n : 3n; // mono: 0x4, stereo: 0x3
        }
    ...
    const filteringCtx: FilteringContext = {
        buffersrc_ctx,
        buffersink_ctx,
        filter_graph,
        enc_pkt: null,
    ...
    return { ret: 0, filteringCtx };
}

export async function init_filters(input_ctx: FormatContext, stream_ctx: StreamContext[]): Promise<{ ret: number; filterCtx: FilteringContext[] }> {
    const filter_spec: string[] = [];
    let ret: number;

    const filter_ctx: FilteringContext[] = new Array(input_ctx.nbStreams);
    ...
    for (let i = 0; i < input_ctx.nbStreams; i++) {
        ...
        const codecpar = input_ctx.streams![i].codecpar;
        ...
        const initResult = await init_filter(stream_ctx[i].dec_ctx!, stream_ctx[i].enc_ctx!, filter_spec[i]);
        ret = initResult.ret;
        if (ret) {
            console.log(`init_filter failed for stream ${i} with ret ${ret}`);
            return { ret, filterCtx: filter_ctx };
        }

        // Update the filtering context with the result
        filter_ctx[i] = initResult.filteringCtx;
    ...
    return { ret: 0, filterCtx: filter_ctx };
}

async function encode_write_frame(
    stream_index: number,
    flush: boolean,
    stream_ctx: StreamContext[],
    filter_ctx: FilteringContext[],
...
export async function filter_encode_write_frame(
    frame: Frame | null,
    stream_index: number,
    stream_ctx: StreamContext[],
    filter_ctx: FilteringContext[],
    ...
    while (true) {
        ...
        ret = await encode_write_frame(stream_index, false, stream_ctx, filter_ctx, ofmt_ctx);
...
export async function flush_encoder(
    stream_index: number,
    stream_ctx: StreamContext[],
    filter_ctx: FilteringContext[],
    ofmt_ctx: FormatContext
    ...
    return await encode_write_frame(stream_index, true, stream_ctx, filter_ctx, ofmt_ctx);
}



// Create a cleanup function to properly dispose of resources
export async function cleanup(ctx: TranscodeContext): Promise<void> {
    const { ifmt_ctx, ofmt_ctx, stream_ctx, filter_ctx } = ctx;

    // Free packets and frames
    for (let i = 0; i < stream_ctx.length; i++) {
        if (stream_ctx[i]) {
